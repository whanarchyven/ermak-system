...
| Web Push не приходит / `[sendWebPush] 403` | См. [Web Push 403](#web-push-403). |
...
В логах Convex: `[sendWebPush] error { code: 403 }` — push-сервис (Google/Mozilla) отклонил запрос.

**Частые причины:**

1. **Подписка создана с другим VAPID-ключом** (локальная разработка → потом прод, или пересоздали ключи в Convex).  
   **Решение:** в PWA → Профиль → снова **«Включить уведомления»** (переподписка).

2. **`VAPID_SUBJECT` не совпадает с боевым доменом** (остался `mailto:admin@ermak.local`).  
   **Решение на VPS:**
   ```bash
   cd ~/ermak-system   # корень монорепо
   docker compose run --rm deployer npx convex env set -- VAPID_SUBJECT https://app.example.com
   ```
   Подставьте ваш `SITE_URL` из `.env`. Затем клиенты переподписываются в профиле.

3. **PWA не по HTTPS** — подписка и доставка push требуют `https://`.

Проверка ключей:
```bash
make keys
# VAPID_PUBLIC_KEY в выводе должен совпадать с тем, что видит PWA (ERP → Настройки или customer:getVapidPublicKey)
```

После смены `VAPID_PUBLIC_KEY` / `VAPID_PRIVATE_KEY` все старые подписки в таблице `pushSubscriptions` недействительны — включите уведомления заново.
